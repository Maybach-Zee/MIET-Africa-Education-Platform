...
exports.register = async (req, res) => {
  ...
  // Auto-generate centre_code
  const code = 'CTR-' + Math.random().toString(36).substr(2, 6).toUpperCase();
  try {
...
exports.approve = async (req, res) => {
  const { id } = req.params;
  // In this system, approval might just set is_active true (if you added a pending status)
  try {
